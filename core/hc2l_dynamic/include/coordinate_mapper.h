...
struct NodeCoordinate {
    ...
    NodeCoordinate(road_network::NodeID id, double lat, double lng) 
...
public:
    ...
    bool getRoadSegment(road_network::NodeID source, road_network::NodeID target, RoadSegment& segment) const;
    
    // Calculate haversine distance between two GPS coordinates in meters
    static double calculateDistance(double lat1, double lng1, double lat2, double lng2);
    
    // Get all node coordinates (for debugging)
    const std::vector<NodeCoordinate>& getAllNodes() const { return node_coordinates; }
...
private:
    std::vector<NodeCoordinate> node_coordinates;
