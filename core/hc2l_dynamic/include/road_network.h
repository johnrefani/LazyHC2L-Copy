...
struct Neighbor;
class Graph;
extern double DISRUPTION_THRESHOLD_TAU;
...
private:
    ...
    NodeID parent; // parent node for path reconstruction
...
struct Edge {
    NodeID from;
    NodeID to;
    double base_weight;       // normal weight
    double disrupted_weight;  // updated if disrupted
    bool is_closed = false;   // road closure flag

    // Constructor
    Edge() = default;

    Edge(NodeID a, NodeID b, distance_t w)
        : from(a), to(b), base_weight(w), disrupted_weight(w), is_closed(false) {}

    // For sorting in std::sort or maps
    bool operator<(const Edge& other) const {
        return std::tie(from, to, base_weight) < std::tie(other.from, other.to, other.base_weight);
    }

    // Unified weight getter
    double get_weight(bool use_disruption) const {
        if (use_disruption && is_closed) return std::numeric_limits<double>::infinity();
        return use_disruption ? disrupted_weight : base_weight;
    }
};
...
class Graph
{
    ...
    // run dijkstra with path reconstruction, storing distance and parent results in node_data
    void run_dijkstra_with_parents(NodeID v);
    // reconstruct path from source to target using parent pointers
    std::vector<NodeID> reconstruct_path(NodeID source, NodeID target);
...
public:
    ...
    // returns path between u and v in subgraph with complete node sequence
    std::pair<distance_t, std::vector<NodeID>> get_path_dijkstra(NodeID v, NodeID w, bool weighted);
    ...
    void applyDisruption(NodeID u, NodeID v, double slowdown, bool closed);
