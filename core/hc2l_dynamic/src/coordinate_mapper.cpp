...
bool CoordinateMapper::loadNodeCoordinates(const std::string& nodes_csv_file) {
    ...
    while (std::getline(file, line)) {
        ...
        try {
            ...
            NodeCoordinate coord(node_id, latitude, longitude);
    ...
    file.close();
    std::cout << "Loaded " << node_coordinates.size() << " node coordinates." << std::endl;
    return !node_coordinates.empty();
}

bool CoordinateMapper::loadRoadSegments(const std::string& scenario_csv_file) {
    ...
    file.close();
    std::cout << "Loaded " << road_segments.size() << " road segments with coordinates." << std::endl;
    return !road_segments.empty();
}

double CoordinateMapper::calculateDistance(double lat1, double lng1, double lat2, double lng2) {
...
road_network::NodeID CoordinateMapper::findNearestNode(double latitude, double longitude, double& distance_m) const {
...
bool CoordinateMapper::getNodeCoordinates(road_network::NodeID node_id, double& latitude, double& longitude) const {
    auto it = node_index_map.find(node_id);
    if (it == node_index_map.end()) {
        return false;
    }
    
    const NodeCoordinate& coord = node_coordinates[it->second];
    latitude = coord.latitude;
    longitude = coord.longitude;
    return true;
}

std::string CoordinateMapper::getRoadName(road_network::NodeID source, road_network::NodeID target) const {
    auto it = segment_map.find({source, target});
    if (it != segment_map.end()) {
        return road_segments[it->second].road_name;
    }
    
    // Try reverse direction
    it = segment_map.find({target, source});
    if (it != segment_map.end()) {
        return road_segments[it->second].road_name;
    }
...
bool CoordinateMapper::getRoadSegment(road_network::NodeID source, road_network::NodeID target, RoadSegment& segment) const {
    auto it = segment_map.find({source, target});
    if (it != segment_map.end()) {
    ...
    if (it != segment_map.end()) {
        segment = road_segments[it->second];
        // Swap coordinates for correct direction
        std::swap(segment.source_lat, segment.target_lat);
        std::swap(segment.source_lng, segment.target_lng);
        std::swap(segment.source_id, segment.target_id);
        return true;
    }
